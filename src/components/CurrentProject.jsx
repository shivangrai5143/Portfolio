...
import SkeletonCard, { SkeletonPill } from './ui/SkeletonCard';

// ── Language accent colors ───────────────────────────────────────────────────
const LANG_COLORS = {
  JavaScript: { bg: 'bg-yellow-400/10', border: 'border-yellow-400/30', text: 'text-yellow-400', dot: '#F7DF1E' },
  TypeScript: { bg: 'bg-blue-400/10',   border: 'border-blue-400/30',   text: 'text-blue-400',   dot: '#3178C6' },
  Python:     { bg: 'bg-blue-500/10',   border: 'border-blue-500/30',   text: 'text-blue-400',   dot: '#3776AB' },
  Java:       { bg: 'bg-orange-400/10', border: 'border-orange-400/30', text: 'text-orange-400', dot: '#F89820' },
  'C++':      { bg: 'bg-indigo-400/10', border: 'border-indigo-400/30', text: 'text-indigo-400', dot: '#00599C' },
  Go:         { bg: 'bg-cyan-400/10',   border: 'border-cyan-400/30',   text: 'text-cyan-400',   dot: '#00ADD8' },
  Rust:       { bg: 'bg-red-400/10',    border: 'border-red-400/30',    text: 'text-red-400',    dot: '#CE4A1D' },
};
const DEFAULT_LANG = { bg: 'bg-slate-400/10', border: 'border-slate-400/30', text: 'text-slate-400', dot: '#94a3b8' };
...
// ── Main component ───────────────────────────────────────────────────────────
const CurrentProject = ({ current, loading }) => {
  ...
  const langStyle = LANG_COLORS[lang] || DEFAULT_LANG;
  const pushed = timeAgo(current.pushedAt);
              ...
              <div className="flex flex-col gap-3 shrink-0">
                ...
                {lang && (
                  <div className="flex flex-wrap gap-2">
                    ...
                    {/* Topics (first 3) */}
                    {(current.topics || []).slice(0, 3).map((topic) => (
                      <span
                        key={topic}
