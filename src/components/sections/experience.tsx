...
const ExperienceSection = () => {
  const allEntries: ExperienceType[] = [...experiences, ...education];
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950 py-24 px-4 sm:px-6 lg:px-8 transition-colors duration-300">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="relative">
          ...
          <div className="hidden md:block absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-[2px] bg-gradient-to-b from-blue-500/40 via-violet-500/40 to-gray-200 dark:to-slate-800" />
                ...
                >
                  ...
                  <div className="md:w-1/2">
                    <motion.div
                      ...
                      className={`relative bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 hover:border-gray-300 dark:hover:border-slate-600 rounded-2xl p-7 shadow-lg hover:shadow-xl transition-all duration-300 group ${
                        isRight ? "md:ml-8" : "md:mr-8"
                      }`}
                    >
                      {/* Top row */}
                      <div className="flex items-start justify-between gap-3 mb-1 flex-wrap">
                        <h3 className="text-lg font-bold text-gray-900 dark:text-white group-hover:text-blue-400 transition-colors font-heading">
                          {entry.title}
                        </h3>
                      ...
                      {/* Org */}
                      <div className="flex items-center gap-2 text-gray-600 dark:text-slate-400 text-sm font-medium mb-1">
                        {isWork
                      ...
                      {/* Duration */}
                      <div className="flex items-center gap-2 text-gray-500 dark:text-slate-500 text-xs mb-5">
                        <FaCalendar size={11} />
                        {entry.duration}
                      </div>

                      {/* Divider */}
                      <div className="border-t border-gray-200 dark:border-slate-800 mb-4" />
                      ...
                      <div className="flex flex-wrap gap-2">
                        {entry.tags.map((tag) => (
                          <span
                            key={tag}
                            className="px-3 py-1 bg-gray-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 hover:border-gray-300 dark:hover:border-slate-500 text-gray-700 dark:text-slate-300 text-xs font-medium rounded-full transition-colors cursor-default"
                          >
                            {tag}
                          </span>
                        ))}
